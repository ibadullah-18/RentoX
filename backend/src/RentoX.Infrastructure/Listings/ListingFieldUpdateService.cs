...
public sealed class ListingFieldUpdateService(
    RentoXDbContext dbContext,
    ICategoryFieldQueryService fieldQueryService)
    : IListingFieldUpdateService
{
    public async Task<UpdateListingFieldsResult>
        UpdateAsync(
            ...
            CancellationToken cancellationToken = default)
    {
        ...
        List<ListingFieldValue> newValues = [];
        ...
        return new UpdateListingFieldsResult(
            listing.Id,
            newValues.Count,
            (int)listing.Status,
    ...
    {
        HashSet<Guid> suppliedIds =
            inputs
                .Select(input => input.FieldId)
                .ToHashSet();

        bool missingRequiredField =
            definitions.Any(definition =>
                definition.IsRequired &&
                !suppliedIds.Contains(definition.Id));
